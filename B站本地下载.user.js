...
(function () {
     ...
     * 避免受到 B 站页面的 CORS、CSP 和本地网络访问限制影响。
     */
    function makeRequest(method, url, data = null) {
        return new Promise((resolve, reject) => {
            GM_xmlhttpRequest({
                ...
                onerror(response) {
                    reject(new Error(
                        `无法连接本地服务：${response.statusText || "网络错误"}`
                    ));
                },
    ...
    /**
     * 读取页面中的 aid。
     * unsafeWindow 指向 B 站页面真正的 window。
     */
    function getAid() {
        try {
    ...
    function getSessdata() {
        ...
        try {
            ...
            const sessdataCookie = cookies.find(
                cookie => cookie?.name === "SESSDATA"
            );

            // 兼容原脚本中第一个元素就是 SESSDATA 的情况。
            return String(
                sessdataCookie?.value ??
                cookies[0]?.value ??
                ""
            );
        } catch (error) {
    ...
    async function startDownload(button) {
        ...
        if (!aid) {
            ...
            console.error("[B站本地下载] 未能获取当前视频 aid");
            return;
        }
        ...
        try {
            ...
            const responseText = await makeRequest(
                "GET",
                `${SERVER}/download/add/${encodeURIComponent(aid)}`
            );
            ...
            try {
                ...
                // 如果服务端返回普通文本，兼容 "true"、"ok"、"success"。
                success = /^(true|ok|success)$/i.test(
                    responseText.trim()
                );
            }
            ...
            setTimeout(() => {
                setButtonText(button, DEFAULT_TEXT);
            }, 2_000);
        } catch (error) {
    ...
    function createDownloadButton() {
        if (document.querySelector("#local-download-button")) {
            return true;
        }

        const brother = document.querySelector(
            ".video-toolbar-right-item"
        );

        if (!brother?.parentElement) {
            return false;
        }
        ...
        button.innerHTML = `
            ${SVG}
            <span class="video-complaint-info video-toolbar-item-text">
                ${DEFAULT_TEXT}
            </span>
        `;
        ...
        brother.parentElement.insertBefore(button, brother);

        console.log("[B站本地下载] 下载按钮已添加");

        return true;
    }

    /**
     * B 站页面内容可能异步加载，因此定时等待工具栏出现。
     */
    ...
    function cancelAutoSkip() {
        ...
        button.click();
        console.log("[B站本地下载] 已取消自动跳转");
    }

    setInterval(cancelAutoSkip, 2_000);
