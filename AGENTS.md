...
- `src/server/` — Express 5 backend:
  ...
  - `download/Bilibili.ts` — talks to bilibili.com: video info API, thumbnail/face download, danmaku download (`comment.bilibili.com/<cid>.xml`) and XML→JSON conversion, and spawning `lux`/`ffmpeg` child processes. Downloaded parts land in `repo/<aid>_download/` and the folder is renamed to `repo/<aid>` on success.
